...
<head>
    ...
    <title>Admin Login — Qafila Loyalty</title>
    @vite(['resources/css/app.css'])
</head>
<body class="bg-slate-100 text-slate-800 antialiased">
    <div class="min-h-screen flex items-center justify-center px-4">
        <div class="max-w-sm w-full">
            <h1 class="text-2xl font-bold text-center text-slate-900">Qafila Loyalty</h1>
            <p class="text-center text-sm text-slate-500 mt-1">Admin Sign In</p>

            <form method="POST" action="{{ route('admin.login.attempt') }}" class="mt-8 bg-white border border-slate-200 rounded-xl p-6 space-y-5">
                @csrf
