...
@section('title', 'Edit ' . $agent->name . ' — Qafila Admin')

@section('content')
    <a href="{{ route('admin.agents.index') }}" class="text-sm text-slate-500 hover:text-slate-700">&larr; Back to agents</a>
    <h1 class="text-2xl font-bold text-slate-900 mt-2">Edit {{ $agent->name }}</h1>

    <form method="POST" action="{{ route('admin.agents.update', $agent) }}" class="mt-6 max-w-xl bg-white border border-slate-200 rounded-xl p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <label class="flex items-center gap-2 text-sm text-slate-700">
            ...
            <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300" @checked(old('is_active', $agent->is_active))>
            Account active
        </label>
